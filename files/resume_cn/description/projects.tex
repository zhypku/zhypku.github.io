\resheading{主要项目}
  \begin{itemize}[leftmargin=*]
    \item
      \ressubsingleline{HiveD：多租户GPU集群调度系统}{$\quad$投稿至\textbf{OSDI 2020}~$\cdot$第一作者}{2018.6 -- 2020.7}
      \vspace{-10pt}\\
      {\small
    ...
    \item
      \ressubsingleline{DShuttle：训练数据存储与调度系统}{$\quad$论文准备中~$\cdot$第一作者}{2019.12 -- 2020.7}
      \vspace{-10pt}\\
      {\small
          \textbf{关键词：缓存策略，性能建模，优化算法，Alluxio}

          云上训练平台往往采取计算与存储相分离的设计，而由于数据规模和训练速度的提升，计算和存储集群间有限的网络带宽逐渐成为性能瓶颈。DShuttle为云上训练场景设计了新型的数据缓存策略，并利用深度学习的任务特点对其性能进行建模，从而对缓存、带宽分配以及任务调度进行联合优化，提升任务性能和资源利用率。
          \vspace{-5pt}
          \begin{itemize}[leftmargin=*]
    ...
    \item
      \ressubsingleline{SDPaxos：半分散式状态机副本协议}{$\quad$\textbf{SoCC 2018}~$\cdot$第一作者}{2016.10 -- 2017.5}
      \vspace{-10pt}\\
      {\small
